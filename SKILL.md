...
description: 将ISV多语言补丁包合并到产品补丁包中。当用户需要将多语言补丁包(dm目录下的zip文件)解压、重组并更新datamodel.xml时触发使用。支持自动提取语言代码、创建mul_<语言代码>目录结构、更新应用列表。
---
...
3. 提取语言代码:
   - 从zip文件名中提取最后两个"."之间的字符
   - 示例: CUS.DIGI.V7.0.6.001_0323.TH.zip → 语言代码为 "TH"
...
```
1. 在target_dir/datamodel/下创建目录: mul_<语言代码>
   示例: mul_TH
   
2. 将target_dir/datamodel/下的所有内容（除mul_<语言代码>外）移动到mul_<语言代码>/中
...
2. 解压dm目录下的21个zip文件
3. 创建 `mul_TH` 目录
4. 更新datamodel.xml，包含所有21个应用的item
